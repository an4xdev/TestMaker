...
public class HtmlBuilderService
{
    ...
    public HtmlBuilderService AddBody(string projectName, PageContent pageContent)
    {
        _stringBuilder.Append(
            ...
            $"\n<button id=\"nextTO\">{pageContent.Button1}</button>" +
            $"\n<button id=\"randomTO\">{pageContent.Button2}</button>" +
            $"\n<button id=\"nextTM\">{pageContent.Button3}</button>" +
            $"\n<button id=\"randomTM\">{pageContent.Button4}</button>" +
            $"\n<button id=\"nextO\">{pageContent.Button5}</button>" +
            $"\n<button id=\"randomO\">{pageContent.Button6}</button>" +
            $"\n</div>" +
            $"\n<div class=\"question-container\">" +
            $"\n<div class=\"question\">" +
            $"\n<h3 class=\"questionHPrefix\">Question:</h3>" +
            $"\n<h3 id=\"questionH\"></h3></div>" +
            $"\n<div><h3 class=\"questionHPrefix\">Answer:</h3></div>" +
    ...
    public HtmlBuilderService AddQuestions(List<Question> questions)
    {
        ...
        testOneQuestions.ForEach(q =>
        {
            ...
            q.Answers.ForEach(a =>
            {
                _stringBuilder.Append($"{{ text: \"{a.Answer.Value}\", \ncorrect: ");
        ...
        {
            _stringBuilder.Append($"{{ question: \"{q.QuestionText}\", \nanswers: [");
            q.Answers.ForEach(a =>
            {
                _stringBuilder.Append($"{{ text: \"{a.Answer.Value}\", \ncorrect: ");
