...
public class HtmlBuilderService
{
    ...
    public HtmlBuilderService AddQuestions(List<Question> questions)
    {
        ...
        {
            var lines = q.Answer.Value.Split('\n');
            var convertedNewLines = string.Join("<br>", lines);
            _stringBuilder.Append($"{{ question: \"{q.QuestionText}\", answer: \"{convertedNewLines}\"}},");
        });
        _stringBuilder.Append("];");
